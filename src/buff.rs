    ...
    T: Clone,
{
    ...
    pub fn expect<P>(&mut self, cond: P) -> Option<T>
    where
        P: FnOnce(&T) -> bool,
    {
        let y = self.next()?;
    ...
    /// are equals.
    /// Returns `None` if the buffer is empty
    pub fn expect_eq(&mut self, x: &T) -> Option<()> {
        self.expect(|y| y == x).map(|_| ())
    }
    ...
    pub fn convert_while<U>(&mut self, pre: fn(&T) -> bool, conv: fn(T) -> U) -> Option<Vec<U>> {
        let mut list = vec![conv(self.expect(pre)?)];
        self.save();
        while let Some(l) = self.expect(pre) {
            list.push(conv(l));
            self.resave();
        }
        self.restore();
